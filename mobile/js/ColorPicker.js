// Based on https://github.com/instea/react-native-color-picker
// with https://github.com/nikki93/react-native-color-picker/commit/a453b82a04e1b8e49d5ba8a64c521b16cec2e28c
// and more app-specific changes
...
// We don't really need to wait for these...
Asset.fromModule(require('../assets/hsv_triangle_mask.png')).downloadAsync();
Asset.fromModule(require('../assets/color_circle.png')).downloadAsync();
...
export default class ColorPicker extends React.PureComponent {
  ...
  _onLayout(l) {
    ...
    if (!this.state.pickerSize || Math.abs(this.state.pickerSize - pickerSize) > 1) {
  ...
  componentWillMount() {
    ...
    this._pickerResponder = createPanResponder({
      onStart: ({ x, y }) => {
        this.props.onScrollDisable && this.props.onScrollDisable();
        this._changingHColor = undefined;
        this.refs.pickerContainer &&
          this.refs.pickerContainer.measure((measureX, measureY, width, height, pageX, pageY) => {
      ...
      onEnd: () => this.props.onScrollEnable && this.props.onScrollEnable(),
    });
  }
            ...
            <View>
              <View style={[styles.triangleContainer, computed.triangleContainer]}>
                ...
                <Image
                  ...
                  source={require('../assets/hsv_triangle_mask.png')}
                />
              </View>
              <View
